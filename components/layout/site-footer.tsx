...
export function SiteFooter({ freeShippingOver, categories }: SiteFooterProps) {
  ...
  return (
    <footer className="bg-[#14110c] text-slate-200">
        ...
        <div className="grid gap-10 md:grid-cols-2 xl:grid-cols-5">
              ...
              <div className="flex gap-3 text-slate-200">
                {social.map((link) => (
                  <a
                    ...
                    className="flex h-10 w-10 items-center justify-center rounded-full border border-slate-700 bg-slate-800/80 transition hover:border-brand-500 hover:text-brand-400"
                  >
                    <Globe size={16} />
                  </a>
                ))}
          ...
          <div>
            ...
            <ul className="space-y-3 text-sm text-slate-300">
              <li><Link href="/shop" className="hover:text-brand-400">All Products</Link></li>
              {categories.slice(0, 5).map((category) => (
                <li key={category}>
                  <Link href={`/shop?category=${encodeURIComponent(category)}`} className="hover:text-brand-400">{category}</Link>
                </li>
              ))}
            </ul>
          </div>
          ...
          <div>
            ...
            <ul className="space-y-3 text-sm text-slate-300">
              {supportLinks.map((link) => (
                <li key={link.label}>
                  <Link href={link.href} className="hover:text-brand-400">{link.label}</Link>
                </li>
              ))}
            </ul>
          </div>
          ...
          <div>
            ...
            <ul className="space-y-3 text-sm text-slate-300">
              {contact.address && (
                <li className="flex items-start gap-3"><MapPin size={16} className="mt-1 shrink-0 text-brand-400" /> {contact.address}</li>
              )}
              {contact.phone && (
                <li className="flex items-center gap-3">
                  <Phone size={16} className="text-brand-400" />
                  <a href={`tel:${contact.phone.replace(/\s+/g, "")}`} className="hover:text-brand-400">{contact.phone}</a>
                </li>
              )}
              {contact.email && (
                <li className="flex items-center gap-3">
              ...
              {!hasContactDetails && (
                <li>
                  Reach us through our <Link href="/contact" className="text-brand-400 hover:text-brand-300">contact form</Link>.
                </li>
              )}
            </ul>
          </div>
        ...
        <div className="mt-12 border-t border-slate-800 pt-8">
          <div className="flex flex-col gap-4 text-sm text-slate-400 md:flex-row md:items-center md:justify-between">
            <div className="flex flex-wrap items-center gap-x-6 gap-y-2">
              ...
              {legalLinks.map((link) => (
                <Link key={link.href} href={link.href} className="hover:text-brand-400">{link.label}</Link>
              ))}
            </div>
            <div className="flex flex-wrap items-center gap-6">
              <span className="inline-flex items-center gap-2"><Truck size={16} className="text-brand-400" /> {freeShippingLabel(freeShippingOver)}</span>
              <span className="inline-flex items-center gap-2"><ShieldCheck size={16} className="text-brand-400" /> Secure checkout</span>
            </div>
          </div>
        </div>
      </div>
