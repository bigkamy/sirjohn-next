...
export function ProductCard({
  ...
  return (
    <article className="group overflow-hidden rounded-3xl border border-brand-100 bg-white shadow-[0_18px_45px_rgba(15,23,42,0.06)] transition-all duration-300 hover:-translate-y-1 hover:shadow-[0_20px_55px_rgba(0,0,0,0.12)]">
      ...
      <div className="space-y-4 p-5">
        <div className="flex items-start justify-between gap-3">
          <div>
            <p className="text-xs font-medium uppercase tracking-[0.2em] text-brand-700/80">
              {brand}
            </p>
            <Link href={`/product/${slug}`} className="mt-2 block text-lg font-semibold text-slate-900 hover:text-brand-700">
              {name}
            </Link>
          </div>
        </div>
